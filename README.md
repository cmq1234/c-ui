...
1、将element的input，select，dialog，datepicker,table进行封装全局注册，利用Vue内置标签component，
   动态的绑定这些组件，根据不同的配置更换不同的组件。
...
5、dialog是类似于message的全局弹出组件，采用this.$CDialog(options)方式使用
```
## 安装
```
